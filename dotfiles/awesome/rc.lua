...
awful.screen.connect_for_each_screen(function(s)
    ...
    s.mywibox:setup {
        ...
        { -- Left widgets
            ...
            --mylauncher, -- That weird menu launcher, I'm not sure why it exists
	    ...
	    cpu_widget({
	    	  ...
	    	  step_spacing = 0,
	    	  color = "#434c5e",
		  timeout = 0.75, -- Might be excessive but the CPU isn't doing anything anyway lmao
	    }),
	    widget_separator,
	    date_widget,
	    empty_space_widget,
...
root.buttons(gears.table.join(
    ...
    -- awful.button({ }, 4, awful.tag.viewnext), These two are absolute CRINGE (scrolling in a gap changes workspace)
...
clientkeys = gears.table.join(
   ...
   awful.key({ modkey, "Shift"   }, "a",
              function () awful.spawn("/etc/nixos/scripts/take_screenshot")                 end,
              {description = "take screenshot", group = "client"}),
   awful.key({ modkey, "Mod1"   }, "a",
              function () awful.spawn("/etc/nixos/scripts/view_latest_screenshots")         end,
              {description = "view latest screenshots", group = "client"}),
...
client.
connect_signal("request::titlebars", function(c)
