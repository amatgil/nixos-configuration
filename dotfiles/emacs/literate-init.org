...
#+end_src
*** Theme-ing
I used to use stylix, but it was so bloated and it caused me so much
trouble, I ended up having to nuke it. Honestly, configuring themes per-program/package is so much nicer

This needs a lot more work, but I've been using Zed for code regardless and this strong black/white is honestly not even that bad.
#+begin_src emacs-lisp
...
#+begin_src emacs-lisp
  ...
  (defun cas-split-window-below ()
    "Like split-window-below (default C-x 2), but moves point to the new one"
    (interactive)
    (split-window-below)
    (evil-window-down 1))
  (defun cas-split-window-right ()
    "Like split-window-right (default C-x 3), but moves point to the new one"
    (interactive)
    (split-window-right)
    (evil-window-right 1))

  (global-set-key (kbd "C-x 2") 'cas-split-window-below)
  (global-set-key (kbd "C-x 3") 'cas-split-window-right)
...
#+begin_src emacs-lisp
  ...
  (global-set-key (kbd "M-e") 'embark-act)  ; Both the "Right Click Menu" and the "I've changed my mind" keybind
...
#+begin_src emacs-lisp
  ...
  (define-key evil-motion-state-map (kbd "g d") 'lsp-find-definition)
...
***** Janet
#+begin_src emacs-lisp
  (unless (package-installed-p 'janet-mode) (package-install 'janet-mode))

  (defvar cas-janet-output-buffer-name "*janet-output*")
  (defvar cas-janet-memory-file-name "/tmp/janet-memory")
  (defvar cas-janet-temp-eval-file-name "/tmp/janet-execute")

  (defun cas-remember-janet-sexp ()
    "Push sexp under point (and behind) to \"memory\"."
    (interactive)
    (let ((start (scan-sexps (1+ (point)) -1))
          (end (1+ (point))))
      (append-to-file start end cas-janet-memory-file-name)))

  (defun cas-eval-janet-sexp ()
    "Evaluate sexp under point within the context of current memory (see `cas-remember-janet-sexp`)"
    (interactive)
    (let* ((start (scan-sexps (1+ (point)) -1))
           (end (1+ (point)))
           (out-buf (get-buffer-create cas-janet-output-buffer-name))
           (text-to-eval (buffer-substring start end)))
      (with-temp-file cas-janet-temp-eval-file-name
        (erase-buffer)
        (insert-file-contents cas-janet-memory-file-name) (end-of-buffer)
        (insert "\n")                                     (end-of-buffer)
        (insert "(print ")                                (end-of-buffer)
        (insert text-to-eval)                             (end-of-buffer)
        (insert ")")                                      (end-of-buffer)
        (with-current-buffer out-buf 
          (erase-buffer)
          (async-shell-command (concat "janet " cas-janet-temp-eval-file-name)
                               out-buf)))))

  (defun cas-hide-janet-output-window ()
    "Equivalent to C-x 0 when the buffer is current"
    (interactive)
    (delete-window (get-buffer-window cas-janet-output-buffer-name)))

  (defun cas-clear-janet-memory ()
    (with-temp-file cas-janet-memory-file-name
      (erase-buffer)))


  ;; These fail because 'janet-mode-map' doesn't exist? Very confusing
  ;; (define-key janet-mode-map (kbd "C-c C-r") #'cas-remember-janet-sexp)
  ;; (define-key janet-mode-map (kbd "C-c C-c") #'cas-eval-janet-sexp)
  ;; (define-key janet-mode-map (kbd "C-c C-k C-r") #'cas-clear-janet-memory)
  ;; (define-key janet-mode-map (kbd "C-c C-q") #'cas-hide-janet-output-window)
#+end_src
***** General modes
#+begin_src emacs-lisp
...
#+end_src
*** Self-Documentation
Emacs really is self-documenting (and is the main reason I'm using it
over *vim), this brings it closer to perfection by replacing emacs'
help pages by using =helpful='s.

Note that the built-in =describe-function= includes both functions and
macros. =helpful-function= is functions only, so this uses
=helpful-callable= as a drop-in replacement.

Note also that =f= is used with =describe-function=, as usual, and
=helpful-callable= goes on =F=. This is because the one-sentence summary
is invaluable for exploration. Helpful's shown information is much
better, however, so i'm keeping both.

Note also also that =C-h a= (=apropos=) can be very useful, too!
#+begin_src emacs-lisp
...
*** Info
My own config, over evil-collection's (=g j= and =g k= are mildly depressing; and =^= is a deadkey for me).
