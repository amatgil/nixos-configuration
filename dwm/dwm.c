...
static int bh, blw, blw, bmw = 0; /* bar geometry */
...
drawbar(Monitor *m)
{
	...
	if (m == selmon || 1) { /* The || 1 makes it so status is NOT only drawn on selected monitor */
	...
	for (i = 0; i < LENGTH(tags); i++) {
		...
		for (c = m->clients; c; c = c->next) {
			if (c->tags & (1 << i)) {
				drw_rect(drw, x, 1 + (indn * 2) + counter * linePadding, selmon->sel == c ? 6 : 1, 2, 1, urg & 1 << i);
				indn++;
				counter++;
			}
		}
	...
	// I don't know what the following paragraph does, but it comes from multimon-1
	char custom[4] = {0}; // needs to be +1 of actual size, for some reason
	snprintf(custom, sizeof(custom), "<%d>", m->num);
	w = bmw = TEXTW(custom);
	drw_setscheme(drw, m == selmon ? &scheme[SchemeSel] : &scheme[SchemeNorm]);
	drw_text(drw, x, 0, w, bh, lrpad / 2, custom, 0); // IF ERROR CHECK HERE
...
sendmon(Client *c, Monitor *m)
{
    ...
    c->y = c->mon->my + (c->mon->mh - HEIGHT(c)) / 2;*/ //Uncomment for centering when transfering floating windows to different mon
	if( attachbelow )
		attachBelow(c);
	else
		attach(c);
